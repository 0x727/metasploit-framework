...
namespace SharpRDPCheck
{
    internal class NTLM
    {
        ...
        public  uint NTLMSSP_NEGOTIATE_ALWAYS_SIGN = 0x8000;
        public  uint NTLMSSP_NEGOTIATE_ANONYMOUS = 0x800;
        public  uint NTLMSSP_NEGOTIATE_DATAGRAM = 0x40;
        public  uint NTLMSSP_NEGOTIATE_EXTENDED_SESSION_SECURITY = 0x80000;
        public  uint NTLMSSP_NEGOTIATE_IDENTIFY = 0x100000;
        private const uint NTLMSSP_NEGOTIATE_KEY_EXCH = 0x40000000;
        public  uint NTLMSSP_NEGOTIATE_LM_KEY = 0x80;
        public  uint NTLMSSP_NEGOTIATE_NTLM = 0x200;
        public  uint NTLMSSP_NEGOTIATE_OEM = 2;
        public  uint NTLMSSP_NEGOTIATE_OEM_DOMAIN_SUPPLIED = 0x1000;
        public  uint NTLMSSP_NEGOTIATE_OEM_WORKSTATION_SUPPLIED = 0x2000;
        public  uint NTLMSSP_NEGOTIATE_SEAL = 0x20;
        public  uint NTLMSSP_NEGOTIATE_SIGN = 0x10;
        private const uint NTLMSSP_NEGOTIATE_TARGET_INFO = 0x800000;
        public  uint NTLMSSP_NEGOTIATE_UNICODE = 1;
        private const uint NTLMSSP_NEGOTIATE_VERSION = 0x2000000;
        public  uint NTLMSSP_REQUEST_NON_NT_SESSION_KEY = 0x400000;
        public  uint NTLMSSP_REQUEST_TARGET = 4;
        public  uint NTLMSSP_TARGET_TYPE_DOMAIN = 0x10000;
        public  uint NTLMSSP_TARGET_TYPE_SERVER = 0x20000;
        ...
        public NTLM(NetworkSocket socket, string sUsername, string sPassword, string sDomain, string sHash)
        {
            this.m_Socket = socket;
            this.m_sUsername = sUsername;
            this.m_sPassword = sPassword;
        ...
        private  byte[] computeResponse(byte[] responseKey, byte[] serverChallenge, byte[] clientData, int offset, int length, out byte[] keyExchangeKey)
        {
        ...
        private  void DumpFlags(uint flags)
        {
        }
        ...
        private  byte[] GenerateSealKey(byte[] exportedSessionKey, byte[] constant)
        {
            List<byte> list = new List<byte>();
        ...
        private  byte[] GenerateSignKey(byte[] exportedSessionKey, byte[] constant)
        {
            List<byte> list = new List<byte>();
            list.AddRange(exportedSessionKey);
            list.AddRange(constant);
        ...
        private  byte[] getLMv2Response(byte[] responseKeyNT, byte[] serverChallenge, byte[] clientChallenge)
        {
        ...
        private  byte[] getNTLMv2Response(byte[] responseKeyNT, byte[] serverChallenge, byte[] clientChallenge, byte[] nanos1601, byte[] av_pairs, out byte[] keyExchangeKey)
        {
        ...
        private  byte[] MakeSignature(RC4 SealKey, byte[] SignKey, byte[] message, ref uint sequenceNum)
        {
        ...
        private  byte[] nTOWFv1(string password)
        {
            if (password == null)
            {
        ...
        private  byte[] nTOWFv2(string domain, string username, string password, string hash)
        {
            byte[] byteArray = null;

            if (hash.Length > 0)
                byteArray = ConvertHexStringToBytes(hash);
            else
        ...
        public byte[] ProcessChallenge(byte[] Challenge)
        {
            ...
            byte[] data = nTOWFv2(this.m_sDomain, this.m_sUsername, this.m_sPassword, this.m_sHash);
        ...
        private  bool TestResult(byte[] result, byte[] expected)
        {
