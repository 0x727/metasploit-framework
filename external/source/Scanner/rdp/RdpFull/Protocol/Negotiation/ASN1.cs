...
namespace SharpRDPCheck
{
    internal class ASN1
    {
        private  Dictionary<string, Fixup> m_Fixup = new Dictionary<string, Fixup>();

        protected  void CloseTag(RdpPacket packet, string Identifier)
        {
            UpdateLength(packet, Identifier);
        }

        protected  int ContextTag(int type)
        {
            return (160 + type);
        }
        ...
        protected  int OctetStringTag()
        {
            return 4;
        }

        protected  int ReadInteger(RdpPacket packet)
        {
        ...
        protected  int ReadLength(RdpPacket packet, string Identifier)
        {
            int num;
            byte[] buffer = new byte[4];
            int num2 = packet.ReadByte();
        ...
        protected  int ReadTag(RdpPacket packet, string Identifier)
        {
        ...
        protected  int ReadTag(RdpPacket packet, int ExpectedTag, string Identifier)
        {
            int num = packet.ReadByte();
        ...
        protected  int SequenceTag(int type)
        {
            return (0x30 + type);
        }

        protected  void UpdateLength(RdpPacket packet, string Identifier)
        {
        ...
        protected  void WriteByte(RdpPacket packet, int value)
        {
            packet.WriteByte((byte) value);
        }

        protected  void WriteInteger(RdpPacket packet, int value)
        {
            packet.WriteByte(2);
        ...
        protected  void WriteLength(RdpPacket packet, string Identifier)
        {
            m_Fixup.Add(Identifier, new Fixup(Identifier, packet.Position));
            WriteByte(packet, 0xff);
        }

        protected  void WriteTag(RdpPacket packet, int Tag, string Identifier)
        {
