...
class MetasploitModule < Msf::Post
    ...
    def initialize(info = {})
        super(
            update_info(
                ...
                'Name' => 'FTP爆破模块',
                'Description' => %q{
                本模块调用execute_dotnet_assembly模块在内存中执行ftp爆破程序
                },
                'License' => MSF_LICENSE,
                'Author' => ['3ricK5r'],
                'Arch' => [ARCH_X64],
    ...
    def run
        ...
        ftp_exe_path = ::File.join(Msf::Config.data_directory, 'post', 'scanner', 'ftp.exe')
        ftp_exe_path = ::File.expand_path(ftp_exe_path)
        datastore['DOTNET_EXE'] = ftp_exe_path
        vprint_status("DOTNET_EXE: #{ftp_exe_path}")
