...
class MetasploitModule < Msf::Post
    ...
    def initialize(info = {})
        super(
            update_info(
                ...
                'Name' => 'MYSQL爆破模块',
                'Description' => %q{
                本模块调用execute_dotnet_assembly模块在内存中执行mysql爆破程序
                },
                'License' => MSF_LICENSE,
                'Author' => ['3ricK5r'],
                'Arch' => [ARCH_X64],
        ...
        register_options(
            [
                ...
                OptInt.new('RPORT', [true, '目标端口', 3306]),
    ...
    def run
        ...
        mysql_exe_path = ::File.join(Msf::Config.data_directory, 'post', 'scanner', 'mysql.exe')
        mysql_exe_path = ::File.expand_path(mysql_exe_path)
        datastore['DOTNET_EXE'] = mysql_exe_path
        vprint_status("DOTNET_EXE: #{mysql_exe_path}")
        ...
        unless File.file?(mysql_exe_path)
            fail_with(Failure::BadConfig, "错误：没有找到.net运行程序#{mysql_exe_path}！")
        end
