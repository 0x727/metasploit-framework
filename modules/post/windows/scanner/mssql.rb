...
class MetasploitModule < Msf::Post
    ...
    def initialize(info = {})
        super(
            update_info(
                ...
                'Name' => 'MSSQL爆破模块',
                'Description' => %q{
                本模块调用execute_dotnet_assembly模块在内存中执行mssql爆破程序
                },
                'License' => MSF_LICENSE,
                'Author' => ['3ricK5r'],
                'Arch' => [ARCH_X64],
        ...
        register_options(
            [
                ...
                OptInt.new('RPORT', [true, '目标端口', 1433]),
    ...
    def run
        ...
        mssql_exe_path = ::File.join(Msf::Config.data_directory, 'post', 'scanner', 'mssql.exe')
        mssql_exe_path = ::File.expand_path(mssql_exe_path)
        datastore['DOTNET_EXE'] = mssql_exe_path
        vprint_status("DOTNET_EXE: #{mssql_exe_path}")
        ...
        unless File.file?(mssql_exe_path)
            fail_with(Failure::BadConfig, "错误：没有找到.net运行程序#{mssql_exe_path}！")
        end
